...
- `strategy.py` — `StrategyFactory`: instantiates a concrete `IReasoningStrategy` from a `StrategyConfig`.
- `tool.py` — `ToolFactory`: instantiates a concrete `ITool` from a `ToolConfig`.
